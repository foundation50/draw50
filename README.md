...
- On pressure-capable pens and devices, pressure subtly affects stroke width. The rear eraser works as a traditional spot eraser, removing only the pixels it passes over with an 8× wider footprint (18× at the default size level).
...
- Visible canvas compositing is synchronized to the browser animation frame while pen samples continue to update the backing surface immediately, avoiding display flicker from partial redraws.
