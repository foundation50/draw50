...
- Drawing uses centripetal Catmull-Rom splines converted to cubic Bezier curves, consuming Chrome's coalesced pen samples when available, to produce smooth rounded paths without the overshoot common to uniform splines. Position and pressure changes are filtered, with stronger position filtering on larger strokes, to reduce visible stepping and jitter.
...
- Drawing pages use conservative supersampling (up to 1.25× the display pixel ratio, capped at 20 million backing pixels) before being downsampled for display. This improves raster edge quality while limiting per-page memory use.
